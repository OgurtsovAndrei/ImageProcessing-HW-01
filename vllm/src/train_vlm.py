...
def train_one_epoch(model, processor, train_files, train_labels_dir, optimizer, device, prompt):
    ...
    for img_path in tqdm(train_files, desc="Training", leave=False):
        try:
            ...
            obj_list = [{"name": "macbook", "bbox": [int((b[0]-b[2]/2)*w), int((b[1]-b[3]/2)*h), int((b[0]+b[2]/2)*w), int((b[1]+b[3]/2)*h)]} for b in boxes]
...
def validate(model, processor, val_files, val_labels_dir, device, prompt):
    ...
    for img_path in tqdm(val_files, desc="Validating", leave=False):
        try:
            ...
            obj_list = [{"name": "macbook", "bbox": [int((b[0]-b[2]/2)*w), int((b[1]-b[3]/2)*h), int((b[0]+b[2]/2)*w), int((b[1]+b[3]/2)*h)]} for b in gt_boxes]
            target_text = json.dumps({"objects": obj_list})
            messages_train = [
                {"role": "user", "content": [{"type": "image", "image": image}, {"type": "text", "text": prompt}]},
                {"role": "assistant", "content": [{"type": "text", "text": target_text}]}
            ...
            with torch.no_grad():
                ...
                messages_eval = [{"role": "user", "content": [{"type": "image", "image": image}, {"type": "text", "text": prompt}]}]
                ...
                output_text = processor.batch_decode([g[len(i):] for i, g in zip(inputs_eval["input_ids"], generated_ids)], skip_special_tokens=True)[0]
...
Please provide the bounding box coordinates in JSON format:
{
  "objects": [
    ...
    results_file_path = "/Users/andrei.ogurtsov/NUP/ImProc/ImageProcessing-HW-01/vllm/src/vlm_fine_tune_results.txt"
    done_sizes = []
        ...
        try:
            ...
            if weights_path.exists():
                ...
                for epoch in range(num_epochs):
                    train_loss = train_one_epoch(model, processor, train_files, train_labels_dir, optimizer, device, prompt)
                    val_metrics = validate(model, processor, val_files[:10], val_labels_dir, device, prompt)
                    print(f"Epoch {epoch+1}/{num_epochs}, Train Loss: {train_loss:.4f}, Val Loss: {val_metrics['val_loss']:.4f}, Val mAP: {val_metrics['map50']:.4f}")
                    history.append({
                        'epoch': epoch + 1,
            ...
            print(f"Size {size} Test mAP@0.5: {test_metrics['map50']:.4f}, Test Mean IoU: {test_metrics['mean_iou']:.4f}")
